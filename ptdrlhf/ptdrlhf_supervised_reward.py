...
def create_trial_data():
    ...
    reward_2 = np.zeros(32) + 1

    state_3 = np.zeros((32, 290))
    action_3 = np.zeros(32) + 1
    reward_3 = np.zeros(32) + 1
    
    np.save(dir_path + "/ptdrlhf" + "/state_action_reward/" + "state_0.npy", state_0)
    np.save(dir_path + "/ptdrlhf" + "/state_action_reward/" + "action_0.npy", action_0)
    np.save(dir_path + "/ptdrlhf" + "/state_action_reward/" + "reward_0.npy", reward_0)

    np.save(dir_path + "/ptdrlhf" + "/state_action_reward/" + "state_1.npy", state_1)
    np.save(dir_path + "/ptdrlhf" + "/state_action_reward/" + "action_1.npy", action_1)
    np.save(dir_path + "/ptdrlhf" + "/state_action_reward/" + "reward_1.npy", reward_1)

    np.save(dir_path + "/ptdrlhf" + "/state_action_reward/" + "state_2.npy", state_2)
    np.save(dir_path + "/ptdrlhf" + "/state_action_reward/" + "action_2.npy", action_2)
    np.save(dir_path + "/ptdrlhf" + "/state_action_reward/" + "reward_2.npy", reward_2)

    np.save(dir_path + "/ptdrlhf" + "/state_action_reward/" + "state_3.npy", state_3)
    np.save(dir_path + "/ptdrlhf" + "/state_action_reward/" + "action_3.npy", action_3)
    np.save(dir_path + "/ptdrlhf" + "/state_action_reward/" + "reward_3.npy", reward_3)
...
def load_batches(n_chunk, batch):
    ...
    action_chunk = action_chunk.astype(int)
    
    #print(f"shape after: {y_chunk.shape}")
    #print(f"action_chunk.max()  {action_chunk.max() }")
    action_hot_encoded_chunk = np.zeros((action_chunk.size, 4))
    action_hot_encoded_chunk[np.arange(action_chunk.size), action_chunk] = 1
    #print(f"action_hot_encoded_chunk {action_hot_encoded_chunk}")
    action_hot_encoded_chunk = action_hot_encoded_chunk.reshape(int(len_chunk/batch), batch, 4)
    ...
    return state_chunk, action_hot_encoded_chunk, reward_chunk
...
class REWARD(nn.Module):
    ...
    #     action_hot_encoded[np.arange(4), action] = 1

    #     with torch.no_grad():
    #         state = torch.cuda.FloatTensor(state).unsqueeze(0)
    #         action_hot_encoded = torch.cuda.FloatTensor(torch.cuda.FloatTensor(state).unsqueeze(0)).unsqueeze(0)
...
def test(net, best_acc, n_test, batch):
    ...
    if accuracy > best_acc:
        torch.save(net.state_dict(), dir_path + "/ptdrlhf/checkpoints/state_action_reward" + '/checkpoint_supervised_reward' + str(round(accuracy*100)) + '.pth')
        return accuracy
    return best_acc
...
def main():
    ...
    if USE_CUDA:
        ...
        state_net = STATENET(290, 4).cuda()

    state_net.load_state_dict(torch.load(dir_path + "/ptdrlhf/checkpoints/state_action" + '/checkpoint_supervised_dqn' + "95" + '.pth'))

    net.layers = copy.deepcopy(state_net.layers)
