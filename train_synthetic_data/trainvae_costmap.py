...
def test():
    ...
    test_loss = 0
    rand_plot = random.randint(0,len(test_loader)-1)
    do_plot = random.randint(0,3)
    with torch.no_grad():
        for batch_idx in range(len(test_loader)):
            data = get_batch(train_loader, batch_idx)
            #print(data[0,2])
            #if batch_idx == rand_plot and do_plot == 0:
            plt.figure()
            imgplot = plt.imshow(data[0,2])
            plt.title('Original costmap')
            plt.show()
            data = data.to(device, dtype=torch.float)
            recon_batch, mu, logvar = model(data)
            ...
            plt.title('Reconstructed costmap')
            plt.show()
...
vae_dir = join(args.logdir, 'vae_inflation')
...
for epoch in range(1, args.epochs + 1):
    ...
    if not args.nosamples:
        with torch.no_grad():
            ...
            #save_image(sample.view(60, 1, RED_SIZE, RED_SIZE),
