...
name_file = 'score_ddqn_mdrnn_e_3_best'
#name_file = 'score3'
...
def main():
	...
	with open(test_file, newline='') as csvfile:
		...
		for row in reader: # For each episode
			...
			value_rl = value_func_rl(value_rl, dist_list, rob_vel_list, vel_list, context_list)
			value = value_rl
		...
		print_by_context(changing_relvel, time_relvel, value, num_contexts)
...
def value_func_bayesian(value, l_1, l_2, l_3, context_list):
	...
	for i in range(length):
		...
		value[context_list[i]].append(rob_vel*(-2 if min_range < 0.75 else 1) -1)

		#value[context_list[i]].append(4*rob_vel*np.tanh((min_range-0.75)/0.2))
	
	return value

def value_func_rl(value, l_1, l_2, l_3, context_list):
	if (len(l_1) is not len(l_2)) or (len(l_2) is not len(l_3)) or (len(l_3) is not len(context_list)):
		assert("Error: lists are not the same size")
	length = len(l_1)
	...
	for i in range(length):
		...
		value_rl[context_list[i]].append(rob_vel*(-2 if min_range < 0.75 else 1) -1)
...
def print_by_context(changing_freq, time_spent, value, num_contexts):
